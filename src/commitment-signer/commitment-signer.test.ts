...
export default class CommitmentSignerTest extends CommitmentSigner {
...
test("should not retrieve a signed commitment if it is not validated by the issuer", async () => {
  ...
  await expect(
    commitmentSignerTest.getSignedCommitment(commitment)
  ).rejects.toEqual(Error("IssuerIdentifier is not valid"));
});
...
test("send retrieve a signed commitment", async () => {
  ...
  const signedCommitment: SignedCommitmentResponse =
    await commitmentSignerTest.getSignedCommitment(commitment);
